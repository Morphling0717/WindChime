...
import type { WindChimeMessageRecord } from "../types.js";
import "../styles/windchime.css";
import { useWindChimeDetail } from "../react/management.js";
import { useWindChimeMutation } from "../react/resource.js";
import { filterWindChimeMessages } from "../core/index.js";
...
const DEFAULT_THEME: Required<WindChimeFlaggedPanelTheme> = {
  ...
  modalTimestamp:
    "mt-1 font-mono text-[11px] text-slate-500 dark:text-slate-400",
  modalContentBox:
    "mt-4 min-h-[120px] rounded-xl border border-rose-200 bg-rose-50/50 p-4 dark:border-rose-400/30 dark:bg-slate-800/60",
  modalLoading:
    "py-6 text-center font-mono text-sm text-rose-500/70 dark:text-rose-300/70",
  modalError:
...
function mergeTheme(
  t?: WindChimeFlaggedPanelTheme,
): Required<WindChimeFlaggedPanelTheme> {
  return {
    ...DEFAULT_THEME,
    ...(t ?? {}),
  };
...
export function WindChimeFlaggedPanel({
  ...
  const flagged = useMemo(
    () => filterWindChimeMessages(items, "flagged"),
    [items],
  );

  const [openId, setOpenId] = useState<string | null>(null);
  const visibleOpenId = flagged.some((item) => item.id === openId)
    ? openId
    : null;
  const detail = useWindChimeDetail(onLoadDetail, { id: visibleOpenId });
  const mutation = useWindChimeMutation(visibleOpenId ?? "");
  const acting = mutation.pending;
  const openData = detail.detail;
  const openLoading = detail.isLoading;
  const openError =
    mutation.mutationError?.message ?? detail.error?.message ?? null;
  const closeModal = useCallback(() => setOpenId(null), []);
  useEffect(() => {
    if (openId && !visibleOpenId) closeModal();
  }, [openId, visibleOpenId, closeModal]);
  ...
  const runAction = async (id: string, fn: () => Promise<void>) => {
    try {
      await mutation.run(fn);
      onAfterAction?.();
      setOpenId((current) => (current === id ? null : current));
    } catch {
      /* The shared mutation state displays the failure and keeps the dialog open. */
    }
  };
