...
async function api(fixture, relative, method = 'GET', body, token, admin = token === undefined) {
  const response = await fetch(fixture.origin + '/api/mail/' + relative, { method, redirect:'error', headers:{ ...(admin?{'x-mail-password':password}:{}), ...(token?{authorization:`Bearer ${token}`} : {}), ...(body===undefined?{}:{'content-type':'application/json'}) }, body:body===undefined?undefined:JSON.stringify(body) });
...
async function startActor(name) {
  ...
  child.on('message',message=>{
    ...
    message.ok ? request.resolve(message.value) : request.reject(Error(`Desktop ${name}: ${message.command} failed`));
  });
...
async function prepare() {
  ...
  for (const [index,origin] of origins.entries()) for (const suffix of ['A','B']) {
    ...
    for(let i=1;i<=2;i++){const posted=await api(fixture,'messages','POST',{topicSlug:slug,text:`PRIVATE_${name}_${i}`,nickname:'合成粉丝',senderFingerprint:randomUUID()},undefined,false);assert(posted.ok);}
...
async function exercise(f,a,b,index) {
  ...
  await a.send('import-ui',{key});const firstSites=await a.call('sites');
  assert.equal(firstSites.items.length,index+1);const selected=firstSites.items.find(i=>i.id===firstSites.selectedId);assert.equal(selected.topicId,f.topicId);
  ...
  assert((await a.send('private')).secretFieldEmpty);assert.deepEqual((await a.send('private')).storedKeys,[]);
  ...
  await a.send('draft-input',{text:'未保存的本机草稿'});
  const stateForConflict=await webState(f), currentDraft=stateForConflict.messages.find(m=>m.id===first).draft;
  await command(f,b,'draft',first,{draft:{...currentDraft,text:f.marker+'_CONFLICT'}});await pause(1300);
  assert.equal(await a.send('draft-value'),'未保存的本机草稿');assert((await a.send('private')).text.includes('其他控制端更新'));
  ...
  assert.equal((await api(f,'live/control/state?topicId='+otherTopic.topicId,'GET',undefined,decoded.token)).status,403);
  ...
  observations.push({name:f.name,hide,scope:{crossTopic:403,crossSite:401},processes:[a.child.pid,b.child.pid]});
...
run().catch(error=>{stage='failed';report({passed:false,stage,error:error.message,directory});process.exitCode=1;}).finally(async()=>{
  ...
  for(const f of fixtures)try{const grants=await admin(f,'live/control/grants?topicId='+f.topicId);for(const grant of grants.items)if(!grant.revokedAt)await admin(f,`live/control/grants/${grant.id}?topicId=${f.topicId}`,'DELETE');}catch{}
