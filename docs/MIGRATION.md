...
网站与桌面固定升级到 0.7.0。`mail_live_grants` 增加显式 `scope`，站点管理授权的 `topic_id` 为 NULL，展示授权仍限定一个话题。SQLite 升级在既有迁移事务内重建授权表以去除旧 NOT NULL 限制，逐行保留 ID、哈希、期限、撤销和父子关系；旧授权全部保持 `topic`，不自动提升权限。迁移记录为 `0.7.0-site-control`。原信件、话题、已读、收藏、词库、批准及排序保留；重新启动不恢复正在展示的内容。

违禁词开关保存于 `mail_settings` 的 `mail.blocked_terms_enabled`，缺失即 false；既有词库不自动启用。关闭时只改变查询与匹配行为，不批量修改历史信件标记。网站 `/mail` 显示密钥管理，旧 `/mail/live` 跳转 `/mail`。新密钥需要桌面 0.7.0；旧密钥继续可用，但只能管理原话题。

用数据库及媒体副本连续迁移两次，核对原列逐行摘要、授权 scope、父子撤销、完整性及外键检查。上线前保留最新一致数据库备份、媒体、环境配置、未纳入 Git 的资源与旧镜像。继续使用原绝对挂载和哈希盐；不要在 release 目录使用相对卷创建空库。回退代码前先在副本测试旧镜像兼容性；不要因代码回退用旧备份覆盖生产新写入。详细操作见 [连接说明](CONNECTION-KEYS.md)，当前验收见 [V070-VALIDATION.md](V070-VALIDATION.md)。
