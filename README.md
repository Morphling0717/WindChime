...
**当前代码版本为 0.8.2，已发布[预发行版](https://github.com/Morphling0717/WindChime/releases/tag/v0.8.2)。** 修复慢网络列表、连接保存、旧图片响应干扰及待播队列问题，玻璃安装向导增加中断恢复。六种排版与三种主题、可调字号、固定图片和文字滚动、五类私人磁贴及“下一封”热键继续保留；网页生成全站连接密钥，桌面提供完整信箱管理和直播控制台。通用库及默认 UI 仍可独立使用，MIT 和第三方声明保持。正式最新版仍为 0.8.1；0.8.2 的实际安装往返与直播姬采集尚未验收。

**2026-09-23 补充：0.8.2 安装 EXE 在隔离首次安装实测中失败，暂请使用便携 ZIP。** 问题修复将另发版本，不覆盖本版标签或资产；详情见[验收记录](docs/RELEASE-082.md)。

[0.8.2 验收进度](docs/RELEASE-082.md) · [0.8.1 发行记录](docs/RELEASE-081.md) · [连接密钥](docs/CONNECTION-KEYS.md) · [桌面运行与打包](apps/desktop/README.md) · [开源许可](docs/LICENSES.md) · [排版与主题](docs/DISPLAY-DESIGNS.md) · [私人磁贴与热键](docs/FLOATING-TILES.md) · [迁移说明](docs/MIGRATION.md)
...
在 `apps/desktop` 执行 `npm ci`、`npm start` 可开发运行，执行 `npm run make` 制作 Windows 安装程序和 ZIP。具体产物及操作见[桌面 README](apps/desktop/README.md)，实际通过项和待验证范围见 [0.8.2 验收记录](docs/RELEASE-082.md)。
