...
.wc-display .wc-display-media img{display:block;max-width:100%;width:auto;height:auto;max-height:min(400px,50cqw);object-fit:contain;border-radius:min(var(--wc-display-radius),12px)}
...
.wc-display .wc-display-brand{position:relative;z-index:1;display:flex;align-items:center;gap:12px;font:500 11px/1.4 "Segoe UI",sans-serif;letter-spacing:.22em;margin-bottom:24px;color:var(--wc-display-accent)}
.wc-display .wc-display-brand-line{height:1px;flex:1;background:currentColor;opacity:.35}
.wc-display .wc-display-brand svg{width:30px;height:30px;flex:none}
...
.wc-display[data-theme="mia"] .wc-display-brand{font-family:Georgia,"Times New Roman",serif;letter-spacing:.2em;margin-bottom:24px}
...
.wc-display[data-layout="split"][data-has-media="true"] .wc-display-grid{grid-template-columns:minmax(0,1.2fr) minmax(0,1fr);column-gap:1.2em}
...
.wc-display[data-layout="banner"] .wc-display-media img{max-height:180px}
.wc-display[data-layout="banner"] .wc-display-brand{margin-bottom:16px}
