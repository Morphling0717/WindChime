...
const DEFAULT_THEME: Required<WindChimeTopicStatePageTheme> = {
  root: "relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-white via-slate-50 to-slate-100 px-4 py-10 text-slate-800 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 dark:text-slate-100",
  gridBackground:
    "pointer-events-none absolute inset-0 opacity-20 dark:opacity-40",
  glowBackground:
  ...
  footer:
    "mt-8 font-mono text-[11px] tracking-[0.2em] text-slate-400 dark:text-slate-500",
  ...
  countdownUnit:
    "mt-0.5 font-mono text-[10px] text-slate-500 dark:text-slate-400",
  countdownStarted:
...
export type WindChimeTopicStatePageProps = {
  ...
  copy?: {
    ...
    disabled?: {
      badge?: string;
      heading?: string;
      body?: (ctx: { topicTitle: string }) => ReactNode;
    };
    scheduled?: {
      badge?: string;
      heading?: string;
...
function resolveCopy(args: {
  ...
  const { variant, topicTitle, startsAt, endsAt, icons, copy, formatTime } =
    args;
...
function TimeCell({
  ...
  return (
    <div className={th.countdownCell}>
      <div className={th.countdownValue}>{String(value).padStart(2, "0")}</div>
