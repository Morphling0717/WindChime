...
0.7.0 在网页 `/mail` 生成 30 天可重复使用的全站连接密钥，桌面管理全部现有和未来话题；旧密钥与旧配对保持原单话题权限。操作见 [CONNECTION-KEYS.md](CONNECTION-KEYS.md)。网站必须实际部署 0.7.0，本次执行结果见 [V070-VALIDATION.md](V070-VALIDATION.md)。
...
| 部分 | 代码/入口 | 职责 |
| --- | --- | --- |
| 独立通用库 | `@windchime/embed@0.7.0`，`core`、`server`、`sqlite`、`next`、`client`、可选 `broadcast` 子入口 | 审核、队列、版本、隔离授权、图片快照、断线安全规则 |
| UliUli / Mia | 两站 `/mail`，旧 `/mail/live` 重定向 | 原信箱管理、话题、共享连接密钥管理 |
| 桌面控制端 | `apps/desktop`；在原站点 `/mail` 生成密钥 | 完整信箱管理、话题、分享、设置及直播控制台 |
...
先备份并升级站点到同一 0.7.0 包，在原网站生成密钥并导入桌面，最后打开独立展示窗口。平台配置不参与这个顺序。网关 `/display`、`/h5` 和站点 `/mail/connect` 属于后文的可选模块。
...
**两个现有站点**固定到各自 `vendor/windchime-embed-0.7.0.tgz`。在每站运行 `npm ci`，保留既有 `.env.local`、数据库路径、登录配置和身份盐，再增加以下配置。例如 UliUli 本地端口选用 3401：
...
打开对应站点 `/mail`，沿用该站原有管理员登录。生产构建用 `npm run build -- --webpack`，启动用 `npm start -- --hostname 127.0.0.1 --port <实际端口>`，前面配置 HTTPS 反向代理；不要在两个进程里同时运行同一站点数据库的直播服务。
...
`make` 的目标产物位于 `apps/desktop/out/installers/`：`WindChime-Setup.exe` 和 `WindChime-win32-x64-0.7.0.zip`，另有 Squirrel 更新文件。ZIP 解压后运行 `WindChime.exe`。默认未做 Windows 代码签名；签名证书可通过构建环境单独配置。具体 ASCII 暂存目录、签名变量、设备凭据和测试命令见 [桌面 README](../apps/desktop/README.md)。是否已生成、安装及实机验证应以本轮验证记录为准，不能仅因有 `make` 命令就视为安装验收通过。
