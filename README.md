...
**当前版本 [0.8.1 已发布](https://github.com/Morphling0717/WindChime/releases/tag/v0.8.1)。** 新版增加带明确安装步骤的玻璃风格安装向导，保留项目 MIT 许可与第三方声明，并修复展示重连后的迟到上屏请求和预览宽高不一致。共享库使用发行页的 `.tgz` 安装。六种排版与三种主题、可调字号、固定图片和文字滚动、五类私人磁贴及“下一封”热键继续保留；网页生成全站连接密钥，桌面提供完整信箱管理和直播控制台。通用库及默认 UI 仍可独立使用。包校验和便携启动已通过，实际安装、升级和卸载的验收限制见下方记录。

[0.8.1 验收进度](docs/RELEASE-081.md) · [0.8.0 发行记录](docs/RELEASE-080.md) · [连接密钥](docs/CONNECTION-KEYS.md) · [桌面运行与打包](apps/desktop/README.md) · [开源许可](docs/LICENSES.md) · [排版与主题](docs/DISPLAY-DESIGNS.md) · [私人磁贴与热键](docs/FLOATING-TILES.md) · [迁移说明](docs/MIGRATION.md)
...
网站至少需要部署 0.7.0 才能支持全站密钥，需要部署 0.8.0 才能保存新排版和外观配置；0.8.1 的展示重连修复还需要更新网站共享包。只更新安装包或仓库依赖不会升级线上服务。UliUli 已于 2026-09-20 更新到 0.8.1 并完成公网验证；Mia 已完成代码、本地构建与接口验证，本轮未部署。详情见对应版本记录。

在 `apps/desktop` 执行 `npm ci`、`npm start` 可开发运行，执行 `npm run make` 制作 Windows 安装程序和 ZIP。具体产物及操作见[桌面 README](apps/desktop/README.md)，实际通过项和待验证范围见 [0.8.1 验收记录](docs/RELEASE-081.md)。
