...
export function useWindChimeLiveControl(client: WindChimeLiveClient, topicId: string) {
  ...
  const apply = useCallback((next: WindChimeLiveControlState) => {
    ...
    current.current = next; setState(next); setConnected(true); setConnectionError(null);
  }, [topicId]);
  useEffect(() => {
    const key = ++generation.current;
    current.current = null; setState(null); setConnectionError(null); setOperationError(null); setConnected(false); setPending(false);
    ...
    const poll = async () => {
      ...
      try { const result = await client.state(topicId, request.signal); if (key === generation.current) apply(result); }
      catch (failure) { if (key === generation.current) { setConnected(false); setConnectionError(failure instanceof Error ? failure.message : '连接中断'); } }
  ...
  const actWithResult = useCallback(async (command: Omit<WindChimeLiveAction, 'topicId' | 'expectedRevision' | 'operationId'>): Promise<WindChimeLiveControlState | null> => {
    ...
    const key = generation.current; setPending(true); setOperationError(null);
    try {
      ...
      if (key === generation.current) {
        // Refreshing after a conflict must not erase the explanation or retry
        // a broadcast command. The user decides when to try again explicitly.
        setOperationError(failure instanceof Error ? failure.message : '操作失败');
  ...
  return { state: state?.topicId === topicId ? state : null, error: operationError ?? connectionError, connectionError, operationError, pending, connected: state?.topicId === topicId && connected, act, actWithResult };
}
