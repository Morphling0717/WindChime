...
const DEFAULT_THEME: Required<WindChimeArchivedTopicsDrawerTheme> = {
  ...
  listEmpty:
    "py-10 text-center font-mono text-sm text-slate-500 dark:text-slate-400",
  listError:
  ...
  listLoading:
    "py-10 text-center font-mono text-sm text-violet-500/70 dark:text-violet-300/70",
  ...
  itemSlug:
    "mt-0.5 truncate font-mono text-[11px] text-slate-500 dark:text-slate-400",
...
export function WindChimeArchivedTopicsDrawer({
  ...
  const archivedItems = useMemo(() => {
    if (isControlled)
      return (itemsProp ?? []).filter((t) => t.state === "archived");
    return fetched.filter((topic) => topic.state === "archived");
    ...
    async (topic: WindChimeTopic) => {
      ...
      try {
        ...
        if (onRestore) {
          ...
          if (restoreEndpointBuilder) {
            const custom = createWindChimeClient({
              baseUrl: "",
              getHeaders: () => requestHeaders ?? {},
            });
            restored = await custom.request<WindChimeTopic>(
              restoreEndpointBuilder(topic.id),
              "PATCH",
              { archivedAt: null },
            );
            client.invalidate(["topics"]);
          } else restored = await client.topics.restore(topic.id);
        } else {
