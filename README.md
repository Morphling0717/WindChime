...
**当前代码版本为 0.8.2。** 修复慢网络列表、连接保存、旧图片响应干扰及待播队列问题，玻璃安装向导增加中断恢复。六种排版与三种主题、可调字号、固定图片和文字滚动、五类私人磁贴及“下一封”热键继续保留；网页生成全站连接密钥，桌面提供完整信箱管理和直播控制台。通用库及默认 UI 仍可独立使用，MIT 和第三方声明保持。发布、实际安装往返和生产部署以本版验收记录为准。
...
网站至少需要部署 0.7.0 才能支持全站密钥，需要部署 0.8.0 才能保存新排版和外观配置；0.8.1 的展示重连保护、0.8.2 的队列与失效修复都需要对应网站共享包。只更新安装包或仓库依赖不会升级线上服务。两站已完成 0.8.2 本地构建与接口验证，生产状态另见本版记录；Mia 本轮不部署。
