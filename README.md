...
**当前桌面为 0.8.3 候选版，尚未完成交付验收；共享库及两站固定依赖仍为 0.8.2。** 本次桌面修复安装器 Unicode 快捷方式接口，以及并发保存“下一封”热键时的注册、回滚和磁贴配置写入竞态。六种排版与三种主题、可调字号、固定图片和文字滚动、五类私人磁贴及“下一封”热键继续保留；网页生成全站连接密钥，桌面提供完整信箱管理和直播控制台。通用库及默认 UI 仍可独立使用，MIT 和第三方声明保持。

运行源码 `47b03b2` 的 Windows 桌面及 Ubuntu Node 20/24 CI 全部通过。**最终候选的实际安装、升级、卸载、便携 ZIP 启动、OBS 与直播姬采集尚未通过完整验收；磁贴置顶实测失败项仍待原生复测。** 自动测试和构建结果不代表这些操作已经通过，当前不能作为验收完成的成品。进度见 [0.8.3 草稿记录](docs/RELEASE-DESKTOP-083.md)。
...
[桌面 0.8.3 草稿记录](docs/RELEASE-DESKTOP-083.md) · [0.8.2 验收记录](docs/RELEASE-082.md) · [0.8.1 发行记录](docs/RELEASE-081.md) · [连接密钥](docs/CONNECTION-KEYS.md) · [桌面运行与打包](apps/desktop/README.md) · [开源许可](docs/LICENSES.md) · [排版与主题](docs/DISPLAY-DESIGNS.md) · [私人磁贴与热键](docs/FLOATING-TILES.md) · [迁移说明](docs/MIGRATION.md)
...
网站至少需要部署 0.7.0 才能支持全站密钥，需要部署 0.8.0 才能保存新排版和外观配置；0.8.1 的展示重连保护、0.8.2 的队列与失效修复都需要对应网站共享包。只更新安装包或仓库依赖不会升级线上服务。两站已完成 0.8.2 本地构建与接口验证。截至 2026-09-23，UliUli 生产仍为 0.8.1：归档并释放旧镜像和缓存后约剩 7.74 GiB，未达到 8 GiB 构建门槛，因此未开始备份、构建或切换。Mia 本轮不部署，详情见[本版记录](docs/RELEASE-082.md)。

在 `apps/desktop` 执行 `npm ci`、`npm start` 可开发运行，执行 `npm run make` 制作 Windows 安装程序和 ZIP。具体产物及操作见[桌面 README](apps/desktop/README.md)，当前候选的实际通过项和待验证范围见 [桌面 0.8.3 草稿记录](docs/RELEASE-DESKTOP-083.md)。0.8.3 不改变网站依赖或数据库迁移版本。
...
从 [0.8.2 预发行版](https://github.com/Morphling0717/WindChime/releases/tag/v0.8.2) 下载共享包并核对同页 `SHA256SUMS.txt`，或在风铃目录执行 `npm pack` 后安装本地生成的包：

```bash
npm install /你的路径/WindChime/windchime-embed-0.8.2.tgz sqlite3@6.0.1
# 需要粉丝图片和主播替换图片时
