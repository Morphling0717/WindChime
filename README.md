...
**当前开发版本 0.8.1，发行验收进行中。** 新版增加带明确安装步骤的玻璃风格安装向导，保留项目 MIT 许可与第三方声明，并修复展示重连后的迟到上屏请求和预览宽高不一致。使用本仓库构建的 `.tgz` 安装。六种排版与三种主题、可调字号、固定图片和文字滚动、五类私人磁贴及“下一封”热键继续保留；网页生成全站连接密钥，桌面提供完整信箱管理和直播控制台。通用库及默认 UI 仍可独立使用。
...
网站至少需要部署 0.7.0 才能支持全站密钥，需要部署 0.8.0 才能保存新排版和外观配置；0.8.1 的展示重连修复还需要更新网站共享包。只更新安装包或仓库依赖不会升级线上服务。本轮两站已完成 0.8.1 代码准备与本地验证，尚未部署；以前的部署见对应版本记录。
